...
Router.get(
  ...
  [
    ...
    body("password", "لطفآ پاسورد وارد کنید و حدافل ۸ رقم باشد")
      .not()
      .isEmpty(),
  ],
  async (req, res) => {
...
Router.post(
  "/add",
  [
    body("name", "لطفآ اسم تانرا وارد کنید")
      .not()
      .isEmpty()
      .trim()
      .escape()
    ...
    body("password", "لطفآ پاسورد وارد کنید و حدافل ۸ رقم باشد")
      ...
      .isEmpty()
      .isLength({ min: 8 }),
    body("company", "لطفآ اسم کمپنی تانرا وارد کنید")
      .not()
      .isEmpty()
      .trim()
      .escape()
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (User) {
        return res.json({
          error:
            "این ایمیل قبلآ در سیستم موجود است لطفآ ایمیل دیگری را استفاده کنید",
        });
      }
...
Router.get("/disabledusers", async (req, res) => {
  try {
    const allUsers = await UserModel.find({ status: "disable" }).populate(
      "contract"
    );
    return res.json(allUsers);
  } catch (error) {
...
Router.post("/contract/add", async (req, res) => {
