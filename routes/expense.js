...
Router.get("/", bussinessAdminMiddleware, async (req, res) => {
    try {
        const allTransactions = await ExpenseModel.find({ user: req.user.id, status: "active" });
...
Router.get("/deleted", bussinessAdminMiddleware, async (req, res) => {
    try {
        const allTransactions = await ExpenseModel.find({ user: req.user.id, status: "deleted" }).populate("customer", "name");
        return res.json(allTransactions);
    } catch (error) {
...
Router.post(
    ...
    [body("date", "لطفآ کارمند را بنوسید").not().isEmpty(), body("details", "لطفآ تفصیلات را بنوسید").not().isEmpty(), body("amount", "لطفآ مقدار پول را بنوسید").not().isEmpty()],
    async (req, res) => {
        ...
        try {
            const newCustomer = new ExpenseModel({ user: userid, date, details, amount, kind });
...
Router.put("/update/:id", bussinessAdminMiddleware, async (req, res) => {
    // destructure all variables
    const { customer, date, details, amount, type, kind } = req.body;

    // create new transaction object to just append the changed fields
    const newCustomer = { customer, date, details, amount, type, kind };
    try {
        ...
        const updatedTransaction = await ExpenseModel.findByIdAndUpdate(req.params.id, newCustomer, { new: true });
...
Router.delete("/delete/:id", bussinessAdminMiddleware, async (req, res) => {
    ...
    try {
        ...
        await ExpenseModel.updateOne({ _id: cusID }, { $set: { status: "deleted" } });
...
Router.delete("/undelete/:id", bussinessAdminMiddleware, async (req, res) => {
    ...
    try {
        ...
        await ExpenseModel.updateOne({ _id: cusID }, { $set: { status: "active" } });
        return res.json({ result: "Successfully Deleted" });
    } catch (error) {
        return res.status(500).send({ error: "Some error occured" });
    }
